...
A thread goal is active. Per-turn usage accounting arrives with goal continuation messages, not here, so this prefix stays stable across turns. If the goal is achieved and no required work remains, call update_goal with status "complete". Do not mark it complete merely because you are stopping or a budget is nearly exhausted. If the goal is genuinely blocked, use update_goal with status "blocked" only after the same blocking condition has repeated for at least three consecutive goal turns and you cannot make meaningful progress without user input or an external-state change. Use the get_goal tool when you need current usage numbers.`;
}
...
export function statusAfterObjectiveEdit(status: GoalStatus): GoalStatus {
	switch (status) {
		case "complete":
		case "budgetLimited":
			return "active";
